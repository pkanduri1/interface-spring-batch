...
public class ConfigurationServiceImpl implements ConfigurationService {
    ...
    public List<SourceSystem> getAllSourceSystems() {
        ...
        return Arrays.asList(
            ...
            new SourceSystem("shaw", "SHAW System", "SQLSERVER", "SHAW Processing", true, 1, LocalDateTime.now(), null)
        );
    }
    ...
    public List<JobConfig> getJobsForSystem(String systemId) {
        ...
        switch (systemId.toLowerCase()) {
            case "hr":
                return Arrays.asList(
                    new JobConfig("hr-p327", "hr", "p327", "HR P327 Processing", 
                        "/input/hr", "/output/hr", "SELECT * FROM hr_table", 
                        true, LocalDateTime.now(), Arrays.asList("200", "900")),
                    new JobConfig("hr-p329", "hr", "p329", "HR P329 Processing", 
                        "/input/hr", "/output/hr", "SELECT * FROM hr_table", 
                        true, LocalDateTime.now(), Arrays.asList("200"))
                );
            case "dda":
                return Arrays.asList(
                    new JobConfig("dda-accounts", "dda", "accounts", "Account Processing", 
                        "/input/dda", "/output/dda", "SELECT * FROM dda_accounts", 
                        true, LocalDateTime.now(), Arrays.asList("200", "300", "900"))
                );
            case "shaw":
                return Arrays.asList(
                    new JobConfig("shaw-processing", "shaw", "processing", "Main Processing", 
                        "/input/shaw", "/output/shaw", "SELECT * FROM shaw_data", 
                        true, LocalDateTime.now(), Arrays.asList("200"))
                );
            default:
    ...
    public List<SourceField> getSourceFields(String systemId, String jobName) {
        ...
        return Arrays.asList(
            new SourceField("customer_id", "STRING", "Customer ID", false, 10, null, null),
    ...
    public String saveConfiguration(FieldMappingConfig config) {
        // TODO: Implement database save + YAML generation
        String configId = config.getSourceSystem() + "-" + config.getJobName() + "-" + System.currentTimeMillis();
        
        // 1. Save to database with audit trail
        // 2. Generate YAML file
        // 3. Write to batch-sources directory
